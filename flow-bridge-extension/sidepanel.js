...
function closeActiveProject() {
  if (currentActiveGemTabId && currentActiveFlowTabId) {
    if (confirm('Close both tabs of this active project?')) {
      [currentActiveGemTabId, currentActiveFlowTabId].forEach(id => {
...
function closeProjectTabs(gemTabId, flowTabId) {
  if (confirm('Close both tabs of this project?')) {
    [gemTabId, flowTabId].forEach(id => {
      if (id) chrome.tabs.remove(id, () => { const _ = chrome.runtime.lastError; });
    });
    setTimeout(loadProjects, 300);
  }
    ...
    chrome.tabs.query({}, (tabs) => {
      chrome.tabs.query({ active: true, lastFocusedWindow: true }, (currentTabs) => {
        ...
        if (currentTab) {
          ...
          if (matchedPair) {
            const gemTab = tabs.find(t => t.id === matchedPair.gemTabId);
            const flowTab = tabs.find(t => t.id === matchedPair.flowTabId);
            if (gemTab) targetGemUrl = gemTab.url;
            if (flowTab) targetFlowUrl = flowTab.url;
          }
        }
        ...
        if (!targetGemUrl || !targetFlowUrl) {
          ...
          const flowTabs = tabs.filter(t => t.url && t.url.includes('labs.google/fx/tools/flow'))
                             .sort((a,b) => (b.active ? 1 : 0) - (a.active ? 1 : 0));
          if (gemTabs.length > 0) targetGemUrl = gemTabs[0].url;
          if (flowTabs.length > 0) targetFlowUrl = flowTabs[0].url;
        }

        if (targetGemUrl && targetFlowUrl) {
          document.getElementById('gemUrl').value = targetGemUrl;
          document.getElementById('flowUrl').value = targetFlowUrl;
...
function renderProjects(projects) {
  ...
  projects.forEach((proj, index) => {
    ...
    closeCardBtn.innerHTML = ICONS.power;
    closeCardBtn.title = 'Close Project';
        ...
        chrome.storage.sync.get(['flowProjects'], (result) => {
          ...
          if (currentTab) {
            ...
            if (matchedPair) {
              ...
              const matchedProject = projects.find(p => {
                ...
                const flowFragment = urlToFragment(p.flowUrl);
                return tabUrl.includes(gemFragment) || tabUrl.includes(flowFragment);
              });
              if (matchedProject) {
          ...
          document.querySelectorAll('.project-card').forEach((card, index) => {
            ...
            if (!gemTab || !flowTab) {
              ...
              gemTab = tabs.find(t => !claimedTabIds.has(t.id) && t.url && t.url.includes(gemFragment));
              flowTab = tabs.find(t => !claimedTabIds.has(t.id) && t.url && t.url.includes(flowFragment));
            }

            if (gemTab && flowTab) {
              claimedTabIds.add(gemTab.id);
              ...
              if (currentActiveProjectIndex === null || proj.id === targetProjectId) {
                ...
                currentActiveGemTabId = gemTab.id;
                currentActiveFlowTabId = flowTab.id;
              }
